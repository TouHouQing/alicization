  ...
  execute: async (botContext, chatCtx, args): Promise<ActionResult> => {
    ...
    if (!unreadEventsForThisChannel || unreadEventsForThisChannel.length === 0) {
      ...
      return {
        success: true,
        shouldContinue: true,
        result: 'Alicization System: No unread messages found.',
      }
    }
    ...
    return {
      success: true,
      shouldContinue: true,
      result: `Alicization System: Read ${unreadEventsForThisChannel.length} unread events from channel ${channelId}:\n${formattedMessages}`,
    }
  },
}
