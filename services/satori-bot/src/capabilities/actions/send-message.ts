    ...
    execute: async (ctx, chatCtx, args) => {
      ...
      if (ctx.unreadEvents[channelId] && ctx.unreadEvents[channelId].length > 0) {
        ...
        return {
          success: false,
          shouldContinue: true,
          result: 'Alicization System: [INTERRUPT] Message sending ABORTED. New unread messages were detected from the user. Please [read_unread_messages] first to understand the new context.',
        }
      }
      ...
      try {
        ...
        // Logic 3: Persistence
        await recordMessage(channelId, chatCtx.selfId, 'Alicization', content)

        return {
          success: true,
          shouldContinue: true,
          result: `Alicization System: Message sent to ${channelId}: ${content}`,
        }
      }
      catch (error) {
        ...
        return {
          success: false,
          shouldContinue: true,
          result: `Alicization System: Error sending message: ${(error as Error).message}`,
        }
      }
    },
  }
