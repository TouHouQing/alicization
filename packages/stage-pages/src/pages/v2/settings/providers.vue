...
import { PaneArea } from '@proj-alicization/stage-ui/components'
import { listProviders } from '@proj-alicization/stage-ui/libs'
import { useProviderCatalogStore } from '@proj-alicization/stage-ui/stores/provider-catalog'
...
<template>
  <div h-full w-full flex flex-col gap-2>
    <h2 text="neutral-900 dark:neutral-100" mb-1 flex justify-between>
      {{ t('settings.pages.providers.catalog_v2.title') }}
    </h2>
          ...
          <div relative h-full w-full flex flex-col gap-2>
            <div relative flex flex-1 flex-col gap-2>
              <div :class="['flex flex-row items-center gap-2']">
                <Input :placeholder="t('settings.pages.providers.catalog_v2.search_configured')" variant="primary-dimmed" />

                <DropdownMenuRoot>
                  <DropdownMenuTrigger as-child :aria-label="t('settings.pages.providers.catalog_v2.actions.customize_options')">
                    <Button size="sm">
                      <div i-ph:plus-light />
                    </Button>
                  </DropdownMenuTrigger>
                  ...
                  <DropdownMenuPortal>
                    <DropdownMenuContent
                      ...
                      align="start"
                    >
                      <Input v-model="availableProviderSearchQuery" :placeholder="t('settings.pages.providers.catalog_v2.search_supported')" class="mb-2" variant="primary-dimmed" />
              ...
              <div v-if="Object.keys(providerCatalogStore.configs).length === 0" class="text-neutral-500 <lg:px-4" flex flex-1 flex-col items-center justify-center gap-2>
                ...
                <div flex items-center justify-center gap-2>
                  <span>{{ t('settings.pages.providers.catalog_v2.empty') }}</span>
                </div>
              </div>
