...
describe('stage embodiment diagnostics', () => {
  ...
  it('keeps signature and reason tags visible without promoting them into residentMode', () => {
    const now = Date.now()
    const diagnostics = useStageEmbodimentDiagnostics({
      activePresence: ref(null),
      live2dExecutionDiagnostics: ref(null),
    ...
    expect(diagnostics.snapshot.value.performance.runtimeDynamics.companionshipTransition.residentMode).toBeNull()
    expect(diagnostics.snapshot.value.performance.runtimeDynamics.companionshipTransition.signature).toBe(
    ...
    expect(diagnostics.snapshot.value.performance.runtimeDynamics.companionshipTransition.settleSummary).not.toContain(
      'mode=',
    )
  })

  it('normalizes a vrm settle_idle action cue without inventing a residentMode', () => {
    const now = Date.now()
    const diagnostics = useStageEmbodimentDiagnostics({
      activePresence: ref(null),
      live2dExecutionDiagnostics: ref(null),
