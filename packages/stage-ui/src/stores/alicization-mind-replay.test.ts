...
describe('alicization mind replay store', () => {
  it('exposes neutral memory-closure diagnostics', () => {
    const source = readFileSync(new URL('./alicization-mind-replay.ts', import.meta.url), 'utf8')

    expect(source).toContain('benchmarkMemoryClosureLongRunSummary')
    expect(source).toContain('benchmarkMemoryClosureRepairTargetRows')
    expect(source).toContain('memoryClosureCausalLane')
  })
  ...
  it('drills into memory-closure repair targets that are not gate failures', async () => {
    const listMindTurnEvents = vi.fn().mockResolvedValue([
      {
        id: 'evt-memory-closure',
        decisionTraceId: 'mind:memory-closure:2',
        turnId: 'turn-memory-closure-2',
        sessionId: 'session-memory-closure',
        origin: 'user-turn',
        kind: 'persistence-written',
        payload: null,
        createdAt: 200,
    ...
    const listMemoryDecisionTraces = vi.fn().mockResolvedValue([
      {
        ...
        sessionId: 'session-memory-closure',
        origin: 'user-turn',
        activeThreadId: 'thread-memory-closure',
        createdAt: 190,
        lastUpdatedAt: 210,
    ...
    store.benchmarkReport = {
      ...
      datasetFeedback: {
        runtimeSamplingEvidence: {
          repairTargets: [{
            lane: 'emotion',
            affectedSessionIds: ['session-memory-closure'],
            sampleTurnIds: ['turn-memory-closure-2'],
            reasons: ['missing causal emotion evidence'],
          }],
          tracePointers: [{
            sampleTurnId: 'turn-memory-closure-2',
            tracePointer: {
              decisionTraceId: 'mind:memory-closure:2',
            },
          }],
    ...
    await store.drillDownBenchmarkTurn('turn-memory-closure-2')

    expect(store.benchmarkMemoryClosureRepairTargetRows).toEqual([{
      lane: 'emotion',
      sessionId: 'session-memory-closure',
      turnId: 'turn-memory-closure-2',
      decisionTraceId: 'mind:memory-closure:2',
      reasons: ['missing causal emotion evidence'],
      targetKind: 'turn',
      firstCheck: '先检查当前回合情绪状态、情绪残留和转换事件。',
    }])
    expect(store.selectedDiagnosisTurnId).toBe('turn-memory-closure-2')
    expect(listMindTurnEvents).toBeCalledWith({
      decisionTraceId: 'mind:memory-closure:2',
      turnId: undefined,
      limit: 200,
  ...
  it('surfaces causal-lane triage from runtime sampling evidence', () => {
    const store = useAlicizationMindReplayStore()
    store.benchmarkReport = {
      packId: 'sampled-humanlike-memory-v1',
      ranAt: 123,
    ...
    expect(store.benchmarkMemoryClosureRepairTargetRows).toEqual([
      {
        lane: 'embodiment',
        sessionId: 'session-cross-modal-gap',
        turnId: 'turn-cross-modal-gap',
        reasons: [
          'cross-modal embodiment proof has only: body; missing at least one of body, voice, face, motion, lipsync',
        ],
        targetKind: 'turn',
        firstCheck: '先检查当前回合语音、表情、口型、动作和机体状态。',
      },
    ])
    expect(store.benchmarkRegressionTriageRows).toEqual([
      {
        dimension: 'memoryClosureCausalLane:turn:embodiment',
  ...
  it('runs sampled memory-closure proof without changing the selected benchmark pack', async () => {
    const runReplayBenchmark = vi.fn().mockResolvedValue({
      packId: 'sampled-humanlike-memory-v1',
      ranAt: 123,
      turnCount: 0,
    ...
    await store.runMemoryClosureProof()
