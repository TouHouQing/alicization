...
describe('alicization self evolution inspector store', () => {
  ...
  it('derives runtime alignment evidence from current visual presence and organic memory state', async () => {
    ...
    const getVisualPresenceState = vi.fn().mockResolvedValue({
      ...
      answerCompiler: {
        ...
        mustDo: [
          ...
          'Let the active self-revision patch require concrete payoff in the same answer.',
        ],
        mustNotDo: [],
      },
      initiative: {
    ...
    expect(store.selectedCandidateRuntimeAlignment).toEqual({
      ...
      response: {
        status: 'aligned',
        expectedSignals: ['hypothesis-labeling', 'specificity-clamp'],
        observedSignals: ['hypothesis-labeling', 'specificity-clamp'],
        confirmedSignals: ['hypothesis-labeling', 'specificity-clamp'],
        missingSignals: [],
        driftingSignals: [],
        reasons: [
  ...
  it('derives partial runtime alignment and effect deltas when current state has not fully converged', async () => {
    ...
    expect(store.selectedCandidateRuntimeAlignment).toEqual({
      ...
      response: {
        ...
        missingSignals: ['hypothesis-labeling'],
        driftingSignals: [],
        reasons: [
  ...
  it('derives persona bias provenance from consumed trace bundle and runtime spine evidence', async () => {
    ...
    expect(store.selectedCandidatePersonaBiasProvenance).toEqual({
      ...
      whySummary: 'persona prefers observe-first room before a closer move.',
      matchedSignals: [
        'personality.relationshipPosture:observer',
        'personality.initiativeStyle:observant',
      ...
      driftingSignals: [],
      reasons: [
        'Consumed trace bundle personality state currently supports relationshipPosture=observer and initiativeStyle=observant.',
        'Consumed trace bundle initiativeBaseline currently supports silenceReconnect=hold and comfortStyle=quiet-presence.',
        'Consumed trace bundle personStateProjection currently supports preferredProactiveStyle=silent-observe.',
  ...
  it('derives private-thought to resident performance projection evidence', async () => {
    ...
    expect(store.selectedCandidateResidentPerformanceProjection).toEqual({
      ...
      driftingSignals: [],
      reasons: [
        'Resident performance still publishes attentive/accompany/soft-covision, so the desk-presence output is preserving the typed resident state.',
  ...
  it('builds a stable candidate impact summary from consumption, runtime alignment, and self evolution evidence', async () => {
    ...
    const getVisualPresenceState = vi.fn().mockResolvedValue({
      ...
      answerCompiler: {
        relationshipPosture: 'restrained',
        mustDo: ['use hypothesis labeling', 'clamp unsupported specificity', 'concrete payoff required'],
      },
      privateThought: {
        stance: 'accompany',
        shouldSpeak: false,
