...
describe('alicization face summary', () => {
  ...
  it('can carry identity-continuity', () => {
    expect(buildAlicizationFaceSummary({
      ...
      'emotion=thinking | cue=soft-gaze | mode=measured-return | blink=linger | gaze=soften | reason=Keep the continuity state inward for now, and leave room before widening outward again',
    )
  })
  ...
  it('can carry an audible-body timing marker when measured-return face cues are still holding the living line as voice and motion rejoin', () => {
    expect(buildAlicizationFaceSummary({
      ...
      'emotion=thinking | cue=soft-gaze | expression=hold | hold=360ms | mode=measured-return | timing=audible-body-carry | blink=linger | gaze=soften | reason=Keep the continuity state audible while face and motion rejoin | src=timeline-projection | conf=0.90 | seg=segment-audible-body-face-1',
    )
  })
})
