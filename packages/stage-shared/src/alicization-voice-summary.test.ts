...
describe('alicization voice summary', () => {
  ...
  it('can carry identity-continuity', () => {
    expect(buildAlicizationVoiceSummary({
      ...
      'zh-CN | companion=measured-return | blink=linger | gaze=soften | reason=Keep the continuity state inward for now, and leave room before widening outward again',
    )
  })
  ...
  it('can surface repair-first concern on the voiced lane when closeness still must not outrun truth and repair', () => {
    expect(buildAlicizationVoiceSummary({
      ...
      'emotion=concerned | companion=repair-before-closeness | timing=next-open-window | blink=quiet | gaze=soften | reason=keep callback facts structured | seg=segment-repair-first-continuity-inward-carry',
    )
  })
})
