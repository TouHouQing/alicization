...
export function buildSelfEvolutionRepairClosure(input: {
  ...
  const bodyContinuityRelevant = bodyContinuityPhase !== null
  ...
  const survivingVisibleLane = input.repairSession.survivingVisibleLane ?? null
  ...
  if (bodyContinuityRelevant && sessionCovered && hasFreshValidationSnapshot && !samePatternStillPresent) {
    summaryLines.push(
      bodyOnlyHold
        ...
        : fullCrossModalLock
          ...
          : rendererRejoinWithoutBody
            ? survivingVisibleLane === 'face+lipsync+voice-only'
              ? '身体连续性已经被新的验证快照再次确认，但当前仍只有表情、口型、声音这条可见通道与同一段表达对齐，body、motion 还没有重新接回这条表情口型声音线，不应把这次较窄的可见延续直接采纳为长期基线。'
              : survivingVisibleLane === 'motion+lipsync+voice-only'
                ? '身体连续性已经被新的验证快照再次确认，但当前仍只有动作、口型、声音这条可见通道与同一段表达对齐，body、face 还没有重新接回这条动作口型声音线，不应把这次较窄的可见延续直接采纳为长期基线。'
                : survivingVisibleLane === 'face+lipsync-only'
                  ? '身体连续性已经被新的验证快照再次确认，但当前仍只有表情、口型这条可见通道与同一段表达对齐，body、motion、voice 还没有重新接回这条表情口型线，不应把这次较窄的可见延续直接采纳为长期基线。'
                  : survivingVisibleLane === 'motion+lipsync-only'
                    ? '身体连续性已经被新的验证快照再次确认，但当前仍只有动作、口型这条可见通道与同一段表达对齐，body、face、voice 还没有重新接回这条动作口型线，不应把这次较窄的可见延续直接采纳为长期基线。'
                    : rendererRejoinSurface
  ...
  return {
    isClosed: sessionCovered && hasFreshValidationSnapshot && !samePatternStillPresent,
    sessionCovered,
    hasFreshValidationSnapshot,
    samePatternStillPresent,
