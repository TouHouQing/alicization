...
export function buildSelfEvolutionFocusHistoryPatternWorkflow(input: {
  ...
  const governanceStepDetail = governanceAnchor === 'persona-thought'
    ...
    : governanceAnchor === 'identity-continuity-continuity'
      ? '先把 identity-continuity 连续性治理当作第一解释归属，避免把记忆先行的熟悉感误判成应该立刻显形的漂移。'
      : governanceAnchor === 'body-continuity'
        ? bodyContinuityPhase === 'full-cross-modal-lock'
          ...
          : bodyContinuityPhase === 'renderer-rejoin-without-body'
            ? survivingVisibleLane === 'face+lipsync+voice-only'
              ? '先把身体连续性治理当作第一解释归属，先确认当前是否仍只有表情、口型、声音这条 identity-continuity 生命线与同一段 living segment 对齐，避免把这次 quieter carry 误判成 body、motion 已经补回的修复完成。'
              : survivingVisibleLane === 'motion+lipsync+voice-only'
                ? '先把身体连续性治理当作第一解释归属，先确认当前是否仍只有动作、口型、声音这条 identity-continuity 生命线与同一段 living segment 对齐，避免把这次 quieter carry 误判成 body、face 已经补回的修复完成。'
                : survivingVisibleLane === 'face+lipsync-only'
                  ? '先把身体连续性治理当作第一解释归属，先确认当前是否仍只有表情、口型这条 identity-continuity 生命线与同一段 living segment 对齐，避免把这次 quieter carry 误判成 body、motion、voice 已经补回的修复完成。'
                  : survivingVisibleLane === 'motion+lipsync-only'
                    ? '先把身体连续性治理当作第一解释归属，先确认当前是否仍只有动作、口型这条 identity-continuity 生命线与同一段 living segment 对齐，避免把这次 quieter carry 误判成 body、face、voice 已经补回的修复完成。'
        ...
        : governanceAnchor === 'project-state-continuity'
          ? '先把项目状态连续性治理当作第一解释归属，避免把项目身份、Phase 1 主线和未闭环 open loops 的丢失误判成普通 identity-continuity 情绪漂移。'
          : '先把显形权威当作第一修复归属，不要过早把问题扩散到人格层。'

  const firstChecklistLine = governanceAnchor === 'persona-thought'
    ? '修复解释仍然锚定在人格/思绪证据上，而不是只剩显形侧后果。'
    : governanceAnchor === 'identity-continuity-continuity'
      ? '连续性解释仍然锚定在 remembered familiarity / identity-continuity room / bounded-growth 上，而不是滑回一般性漂移修复。'
      : governanceAnchor === 'body-continuity'
        ? bodyContinuityPhase === 'full-cross-modal-lock'
          ? `连续性解释仍然锚定在身体线 / ${rendererRejoinSurfaceLabel} / 同一段 living segment 共同锁定上，而不是滑回普通显形权威漂移。`
          : bodyContinuityPhase === 'renderer-rejoin-without-body'
            ? survivingVisibleLane === 'face+lipsync+voice-only'
              ? '连续性解释仍然锚定在表情、口型、声音这条 identity-continuity 生命线上，而不是把这次 quieter carry 写成 body、motion 已经回接。'
              : survivingVisibleLane === 'motion+lipsync+voice-only'
                ? '连续性解释仍然锚定在动作、口型、声音这条 identity-continuity 生命线上，而不是把这次 quieter carry 写成 body、face 已经回接。'
                : survivingVisibleLane === 'face+lipsync-only'
                  ? '连续性解释仍然锚定在表情、口型这条 identity-continuity 生命线上，而不是把这次 quieter carry 写成 body、motion、voice 已经回接。'
                  : survivingVisibleLane === 'motion+lipsync-only'
                    ? '连续性解释仍然锚定在动作、口型这条 identity-continuity 生命线上，而不是把这次 quieter carry 写成 body、face、voice 已经回接。'
        ...
        : governanceAnchor === 'project-state-continuity'
          ? '连续性解释仍然锚定在项目身份 / Phase 1 本地主continuity evidence / 未闭环 open loops 上，而不是滑回泛化的 identity-continuity 漂移修复。'
  ...
  const eventAuditDetail = governanceAnchor === 'identity-continuity-continuity'
    ...
    : governanceAnchor === 'body-continuity'
      ? bodyContinuityPhase === 'full-cross-modal-lock'
        ...
        : bodyContinuityPhase === 'renderer-rejoin-without-body'
          ? survivingVisibleLane === 'face+lipsync+voice-only'
            ? `优先检查 ${eventList}，找到这次表情、口型、声音 identity-continuity 存活线反复出现背后最早没能让 body、motion 重新接回的断点。`
            : survivingVisibleLane === 'motion+lipsync+voice-only'
              ? `优先检查 ${eventList}，找到这次动作、口型、声音 identity-continuity 存活线反复出现背后最早没能让 body、face 重新接回的断点。`
              : survivingVisibleLane === 'face+lipsync-only'
                ? `优先检查 ${eventList}，找到这次表情、口型 identity-continuity 存活线反复出现背后最早没能让 body、motion、voice 重新接回的断点。`
                : survivingVisibleLane === 'motion+lipsync-only'
                  ? `优先检查 ${eventList}，找到这次动作、口型 identity-continuity 存活线反复出现背后最早没能让 body、face、voice 重新接回的断点。`
  ...
  const validateDetail = governanceAnchor === 'identity-continuity-continuity'
    ...
    : governanceAnchor === 'body-continuity'
        ...
        : bodyContinuityPhase === 'renderer-rejoin-without-body'
          ? survivingVisibleLane === 'face+lipsync+voice-only'
            ...
            : survivingVisibleLane === 'motion+lipsync+voice-only'
              ...
              : survivingVisibleLane === 'face+lipsync-only'
                ? '改动后抓取新的聚焦快照，确认新的转移不再把当前只有表情、口型这条 identity-continuity 生命线仍在维持的 quieter carry 误判成修复完成。'
                : survivingVisibleLane === 'motion+lipsync-only'
                  ? '改动后抓取新的聚焦快照，确认新的转移不再把当前只有动作、口型这条 identity-continuity 生命线仍在维持的 quieter carry 误判成修复完成。'
      ...
      : governanceAnchor === 'project-state-continuity'
        ? '改动后抓取新的聚焦快照，确认项目身份、Phase 1 本地主continuity evidence和未闭环 open loops 仍被身份连续性稳定带着，而不是再次在回放里掉线。'
  ...
  const headline = governanceAnchor === 'body-continuity'
    ? bodyContinuityPhase === 'full-cross-modal-lock'
      ...
      : bodyContinuityPhase === 'renderer-rejoin-without-body'
        ? survivingVisibleLane === 'face+lipsync+voice-only'
          ? `${repeatedTransitions} 次反复转移共享同一表情、口型、声音 identity-continuity 存活线治理特征。`
          : survivingVisibleLane === 'motion+lipsync+voice-only'
            ? `${repeatedTransitions} 次反复转移共享同一动作、口型、声音 identity-continuity 存活线治理特征。`
            : survivingVisibleLane === 'face+lipsync-only'
              ? `${repeatedTransitions} 次反复转移共享同一表情、口型 identity-continuity 存活线治理特征。`
              : survivingVisibleLane === 'motion+lipsync-only'
                ? `${repeatedTransitions} 次反复转移共享同一动作、口型 identity-continuity 存活线治理特征。`
  ...
  return {
    ...
    steps: [
      ...
      {
        key: 'validate',
        title: '修复后验证 identity-continuity 连续性',
            ...
            : bodyContinuityPhase === 'renderer-rejoin-without-body'
              ? survivingVisibleLane === 'face+lipsync+voice-only'
                ...
                : survivingVisibleLane === 'motion+lipsync+voice-only'
                  ...
                  : survivingVisibleLane === 'face+lipsync-only'
                    ? '新的快照不再把当前只有表情、口型这条 identity-continuity 生命线仍在维持的 quieter carry 误写成修复完成。'
                    : survivingVisibleLane === 'motion+lipsync-only'
                      ? '新的快照不再把当前只有动作、口型这条 identity-continuity 生命线仍在维持的 quieter carry 误写成修复完成。'
