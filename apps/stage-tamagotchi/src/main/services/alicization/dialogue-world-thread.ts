...
export function buildDialogueWorldThreadSystemBlock(state: AlicizationDialogueWorldThreadSnapshot | null | undefined) {
  ...
  return [
    ...
    'block_role=dialogue_world_thread; owner=WorkingMemory; wording_authority=false',
    `active_thread=${state.activeThread}`,
    `current_question=${state.currentQuestion ?? 'none'}`,
    `primary_turn_anchor=${state.primaryTurnAnchor ?? 'none'}`,
    `primary_turn_anchor_source=${state.primaryTurnAnchorSource ?? 'none'}`,
    `open_loops=${state.openLoops.length > 0 ? state.openLoops.join(' | ') : 'none'}`,
    `recently_resolved_loops=${state.recentlyResolvedLoops.length > 0 ? state.recentlyResolvedLoops.join(' | ') : 'none'}`,
    `carried_facts=${state.carriedFacts.length > 0 ? state.carriedFacts.join(' | ') : 'none'}`,
    `relation_drift=${state.relationDrift}`,
    `memory_mode=${state.memoryMode}`,
    `carry_eligible=${state.carryEligible === true ? 'true' : 'false'}`,
    `carry_reason=${state.carryReason ?? 'none'}`,
    `last_user_move=${state.lastUserMove}`,
    `last_assistant_move=${state.lastAssistantMove ?? 'none'}`,
    `last_outcome=${state.lastOutcome}`,
    `pending_validation=${state.pendingValidation ? JSON.stringify(state.pendingValidation) : 'none'}`,
    `recall_keys=${state.recallKeys.length > 0 ? state.recallKeys.join(' | ') : 'none'}`,
    'surface_residue_authority=non_authoritative_when_thread_anchor_differs',
  ].join('\n')
}
