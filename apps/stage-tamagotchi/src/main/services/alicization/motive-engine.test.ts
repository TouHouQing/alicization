...
describe('buildMotiveEngine', () => {
  it('turns remembered open loops into durable return agendas instead of forgetting them turn by turn', () => {
    ...
    expect(returnAgenda?.summary).toContain('runtime knot')
    expect(motive.backgroundAgendas.map(agenda => agenda.summary).join('\n')).not.toMatch(/preserve-trust|return-open-loop|unfinished_thread_return=/u)
  })
  ...
  it('turns remembered host-confirmed resume confirmation into explicit boundary-restraint motive instead of generic open-loop pressure', () => {
    ...
    expect(resumeBoundaryAgenda?.summary).toContain('host-confirmed resume')
    expect(resumeBoundaryAgenda?.summary).not.toMatch(/remembered_host_confirmed_resume=|host_boundary=/u)
  })
})
