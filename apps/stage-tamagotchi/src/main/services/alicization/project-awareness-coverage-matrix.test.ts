...
const coverageRows = [
  ...
  },
  {
    ...
    evidence: 'front-stage quick-reply closure keeps project identity, landed progress, still-open life-loop pressure, and the explicit emotion-memory-initiative-embodiment same-life seam visible at the last host-facing step before the turn opens outward, so outward wording still comes from one same-her self brief instead of flattening before the host-facing closure summary',
    fileMustContain: [
  ...
  },
  {
    ...
    fileMustContain: [
      ...
      'responsibility).toContain(\'quieter lipsync+voice and body+lipsync same-her carry\')',
      'responsibility).toContain(\'living audio thread\')',
      'responsibility).toContain(\'quieter living line\')',
      'responsibility).toContain(\'visible same-her line has already rejoined without body carry\')',
      'expect(matrixSource).toContain(\'same-living-self-host-visible-inward-carry-bridge-audit.test.ts\')',
  ...
  },
  {
    ...
    evidence: 'renderer diagnostics keep the same-her project line explicit when embodiment narrows under noisy desktop drift, so partial-lane alerts stay on one living audible body line instead of collapsing into a generic warning shell',
    fileMustContain: [
  ...
  },
  {
    ...
    evidence: 'direct bridge dialogue surfaces preserve or intentionally rebuild same-her project awareness across remote channels, so context-bridge forwarding and epoch1 bridge extraction stay on one digital-life line instead of fabricating a detached bridge shell',
    fileMustContain: [
  ...
  },
  {
    ...
    evidence: 'outbound pre-dialogue transport preserves same-her project awareness across renderer send, transport sanitization, and bridge forwarding, so the send identity survives serialization and observation boundaries before the turn opens outward',
    fileMustContain: [
      'keeps one explicit route-level proof that outbound pre-dialogue transport preserves same-her project awareness across renderer send, transport sanitization, and bridge forwarding',
  ...
  },
  {
    ...
    evidence: 'project-state observation rebuilds same-her continuity snapshots before later turns reopen, so thin-shell return-side state still recovers the broader Phase 1 self line, quieter body-led closure carry, closure reasons, and embodiment-open carry before the next outward turn',
    fileMustContain: [
  ...
  },
  {
    ...
    proofKind: 'dynamic',
    file: '../../../../../../packages/stage-ui/src/stores/project-state-observation-project-awareness-audit.test.ts',
    evidence: 'return-side project-state observation rebuild is now covered by the dedicated observation audit, which keeps thin-shell same-her recovery and reopened-turn continuity on one explicit route-level proof chain',
    fileMustContain: [
      'keeps one explicit route-level proof that return-side observation rebuilds same-her continuity snapshots before later turns reopen',
      'expect.objectContaining({ entry: \'observation-thin-shell-same-her-recovery\' })',
      'expect.objectContaining({ entry: \'observation-resume-reasons-carry\' })',
