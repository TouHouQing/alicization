...
describe('runtime proactive self-brief closure regression', () => {
  it('keeps proactive self-brief carrying the Phase 1 digital-life triad and same-her closure obligations before any initiative generation starts', () => {
    ...
    expect(proactiveSelfBriefBlock).toContain('continuity_anchor=')
    expect(proactiveSelfBriefBlock).toContain('continuity_hold=')
    ...
    expect(proactiveSelfBriefBlock).toContain('continuity_drift_risk=')
    expect(proactiveSelfBriefBlock).toContain('proactive_scope=initiative_arbitration')
    expect(proactiveSelfBriefBlock).toContain('project_context=phase1_local_digital_life')
  ...
  it('keeps proactive self-brief ordered as project identity -> phase -> awareness -> same-her -> landed/open/next so initiative reasoning cannot reopen from a thinner shell', () => {
    const source = readFileSync(new URL('./runtime.ts', import.meta.url), 'utf8')
    const blockStart = source.indexOf('function buildProactiveProjectSelfBriefSystemBlock() {')
    const blockEnd = source.indexOf('\n\n  function buildDreamProjectSelfBriefSystemBlock()', blockStart)
    const proactiveSelfBriefBlock = blockStart >= 0 && blockEnd > blockStart
    ...
    const continuityAnchorIndex = proactiveSelfBriefBlock.indexOf('continuity_anchor=')
    const continuityHoldIndex = proactiveSelfBriefBlock.indexOf('continuity_hold=')
    ...
    const driftRiskIndex = proactiveSelfBriefBlock.indexOf('continuity_drift_risk=')
    ...
    expect(continuityAnchorIndex).toBeGreaterThan(preDialogueAwarenessIndex)
    expect(continuityHoldIndex).toBeGreaterThan(continuityAnchorIndex)
    expect(landedIndex).toBeGreaterThan(continuityHoldIndex)
