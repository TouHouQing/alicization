...
describe('codex executor adapter', () => {
  ...
  it('dispatches codex execution and records dispatch, step, and result events', async () => {
      ...
      command: {
        ...
        runtimeContext: {
          ...
          projectBriefing: {
            ...
            latestLandedProgress: 'Execution runtime context already carries same-her project continuity before Codex starts.',
    ...
    expect(execFileMock).toBeCalledWith(
      ...
      expect.arrayContaining([
        ...
        expect.stringContaining('project_preflight=identity=Alicization | phase=Phase 1 | open=execution chain same-line closure'),
        expect.stringContaining('project_awareness=Before dispatch, remember this is still the same local-first digital life project.'),
  ...
  it('allows low-risk autonomous code edits to self-start on codex when planning already marked them as grounded same-her execution', async () => {
    execFileMock.mockImplementation((_command: string, args: string[], _options: unknown, callback: (error: Error | null, stdout?: string, stderr?: string) => void) => {
      const outputPath = args[args.indexOf('--output-last-message') + 1]
      void Promise.resolve()
        .then(async () => {
