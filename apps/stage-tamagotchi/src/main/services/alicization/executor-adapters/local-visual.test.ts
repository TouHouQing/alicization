...
function createRuntimeContext(overrides: Record<string, unknown> = {}) {
  return {
    ...
    projectBriefing: {
      ...
      primaryOpenLoop: 'Delegated desktop execution still needs stronger long-run same-her closure across more real noisy desktop detours.',
      ...
      sameHerSelfLine: 'Same Phase 1 digital life. The delegated desktop execution handoff still belongs to one living her.',
      sameHerHoldDetail: 'same-her hold: delegated desktop execution should stay on the same living line before reinspection returns.',
      sameHerDriftRisk: 'If delegated desktop execution drops back to a generic tool shell, treat it as unfinished same-her closure drift.',
      continuityCue: 'Keep delegated desktop execution on the same living line before local visual reinspection returns.',
      preflightSummary: 'Alicization is a local-first digital life project | Phase 1: Local Digital Life | open=Delegated desktop execution still needs stronger same-her closure across noisy detours. | next=Keep the delegated desktop execution line project-aware before reinspection returns.',
      preDialogueAwarenessLine: 'Before answering, remember this is still the same local-first digital life project and delegated desktop execution must stay on that same living line.',
    },
    sensory: {
...
const expectedDelegatedRuntimeProjectBriefing = expect.objectContaining({
  ...
  primaryOpenLoop: expect.stringContaining('Delegated desktop execution still needs stronger long-run same-her closure'),
