  ...
  it('derives deferred continuity focus summaries from alias-only open and next project-state carry', () => {
    ...
    const signal = runtime.buildDeferredAutonomyContinuitySignal({
      ...
      projectState: {
        ...
        latestLandedProgress: 'Alias-only landed progress already keeps callback continuity on one identity-continuity',
        openClosureSummary: 'Emotion, memory, initiative, and embodiment still need one stronger continuity state closure seam.',
        nextClosureTargetSummary: 'Keep project identity carry, Phase 1 route carry, measured-return initiative, and resident presence on one continuity state.',
        emotionalClosureCue: 'Keep this return low-pressure on the continuity state.',
      } as any,
      autonomy: {
