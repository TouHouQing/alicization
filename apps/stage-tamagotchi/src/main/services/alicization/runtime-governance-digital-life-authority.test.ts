...
import { normalizeDialogueRespondedPayload as normalizeDialogueRespondedPayloadBase } from './runtime-governance'
...
function normalizeDialogueRespondedPayload(
  ...
  return normalizeDialogueRespondedPayloadBase(
    input as RuntimeAlicizationConversationTurnInput,
...
function buildManifest() {
  return {
    ...
    supportedActions: [
      { key: 'idle_gentle_nod', label: 'Idle Gentle Nod', description: 'gentle idle nod', source: 'live2d-motion' },
...
function buildScriptDigitalLifeOnlyTurn() {
  return {
    turnId: 'turn-runtime-governance-script-digital-life-authority',
    sessionId: 'session-runtime-governance-script-digital-life-authority',
    assistantText: '我先沿着这条线轻一点接住。',
    origin: 'user-turn',
    createdAt: 1,
    governance: {
    ...
    structured: {
      ...
      embodimentScript: {
        ...
        digitalLife: {
          ...
          variationToken: 'turn-runtime-governance-script-digital-life-authority',
          emotion: 'thinking',
          mode: 'recovering',
...
describe('runtime-governance digital-life authority', () => {
  it('hydrates normalized digitalLife from embodiment-script authority when top-level digitalLife is absent', () => {
    const dialoguePayload = normalizeDialogueRespondedPayload(
      buildScriptDigitalLifeOnlyTurn(),
      buildManifest(),
    )
    ...
    expect(dialoguePayload?.structured.speechTimeline?.segments).toEqual([
      expect.objectContaining({
        id: 'segment-script-digital-life-1',
        startOffset: 0,
        endOffset: 13,
