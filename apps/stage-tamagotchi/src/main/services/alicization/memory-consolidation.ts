...
function buildConsolidationMetadata(events: AlicizationEpisodicEventRecord[]) {
  const derivedReferences = new Map<string, {
    kind: AlicizationEpisodicEventRecord['derivedFrom'][number]['kind']
    id: string
  }>()
  for (const reference of events.flatMap(event => event.derivedFrom)) {
    const id = sanitizeText(reference.id, 160)
    if (!id)
      continue
    derivedReferences.set(`${reference.kind}:${id}`, {
      kind: reference.kind,
      id,
    })
  }
  ...
  return {
    evidence: {
      ...
      sourceKinds: [...new Set(events.map(event => event.sourceKind))],
      provenances: [...new Set(events.map(event => event.latestReconsolidation?.provenance ?? event.provenance))],
      decisionTraceIds: uniqueList(events.map(event => event.latestReconsolidation?.decisionTraceId ?? event.decisionTraceId), 32),
      turnIds: uniqueList(events.map(event => event.turnId), 32),
      sessionIds: uniqueList(events.map(event => event.sessionId), 32),
      derivedReferences: [...derivedReferences.values()],
      eventStatus: events.map(event => ({
        eventId: event.id,
        sourceKind: event.sourceKind,
        provenance: event.provenance,
        confidence: event.confidence,
        salience: event.salience,
        consolidationPriority: event.consolidationPriority,
        recallCount: event.recallCount,
        reconsolidationCount: event.reconsolidationCount,
        memoryTier: event.memoryTier ?? null,
      })),
      relationshipShifts: events.flatMap(event => event.relationshipShift
        ? [{
            eventId: event.id,
            ...event.relationshipShift,
          }]
        : []),
      reconsolidations: events.flatMap(event => event.latestReconsolidation
        ? [{
            eventId: event.id,
            at: event.latestReconsolidation.at,
            decisionTraceId: event.latestReconsolidation.decisionTraceId ?? null,
            provenance: event.latestReconsolidation.provenance,
            confidence: event.latestReconsolidation.confidence,
          }]
        : []),
    },
...
function buildEvidenceSummary(
  ...
  return uniqueList(sortEvents(events).flatMap(event => [
...
function deriveRecurrentBurden(events: AlicizationEpisodicEventRecord[]) {
  ...
  return burdenCandidates[0] ?? null
}
...
function buildAutobiographicalSummary(input: {
  ...
  return buildEvidenceSummary(
    input.events,
    `consolidation:autobiographical:${input.facet}:${input.periodKey}`,
  )
}
...
function shouldPreserveSingleEventAutobiographicalCarry(input: {
  ...
  if (input.facet === 'relationship-era')
    return event.sourceKind === 'dialogue-feedback' || event.relationshipShift != null

  return event.sourceKind === 'dream'
    || event.sourceKind === 'dream-reforge'
    || event.sourceKind === 'reflection'
    || event.sourceKind === 'maintenance'
}
...
export function buildMemoryConsolidationRecords(input: {
  ...
  for (const [periodKey, bucketEvents] of dailyBuckets) {
    ...
    const record: AlicizationMemoryConsolidationRecord = {
      ...
      summary: buildEvidenceSummary(bucketEvents, `consolidation:daily:${periodKey}`),
  ...
  for (const [periodKey, bucketEvents] of weeklyBuckets) {
    ...
    const weeklyRecord: AlicizationMemoryConsolidationRecord = {
      ...
      periodStartedAt: Math.min(...bucketEvents.map(event => event.occurredAt)),
      periodEndedAt: Math.max(...bucketEvents.map(event => event.occurredAt)),
      summary: buildEvidenceSummary(bucketEvents, `consolidation:weekly:${periodKey}`),
      lesson: buildLesson(bucketEvents),
      cues: uniqueList(sorted.flatMap(event => [
        event.threadAnchor,
        event.whereSummary,
  ...
  for (const item of procedural) {
    ...
    const proceduralRecord: AlicizationMemoryConsolidationRecord = {
      ...
      summary: sanitizeText(item.approach || item.label || `consolidation:procedural:${item.id}`, 280),
