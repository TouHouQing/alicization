...
describe('main chat execution reply obligation', () => {
  it('elevates a fresh execution callback into a direct reply obligation for result follow-ups', () => {
    ...
    expect(systemBlock).toContain('same_her_drift_risk=If project-state continuity survives only as generic guidance while the direct same-her self line disappears, treat that as unfinished closure drift rather than a successful turn.')
