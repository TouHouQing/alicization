...
describe('main chat follow-up payoff', () => {
  ...
  it('answers generic execution-result follow-ups from the real settled task outcome', async () => {
    ...
    expect(structured).toBeTruthy()
    const parsed = JSON.parse(structured!)
    expect(parsed.reply).toContain('CLI 那条任务已经跑完了')
    ...
    expect(parsed.governance.dialogueActKernel.openingClaim).toContain('沿同一条线程继续')
    expect(parsed.governance.dialogueActKernel.mustSay[0]).toContain('沿同一条线程继续')
