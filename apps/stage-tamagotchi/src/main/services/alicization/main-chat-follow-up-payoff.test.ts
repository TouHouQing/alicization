...
describe('main chat follow-up payoff', () => {
  ...
  it('answers generic execution-result follow-ups from the real settled task outcome', async () => {
    ...
    expect(parsed.governance.dialogueActKernel.openingClaim).toMatch(/沿同一条.*(线程|线).*(继续|补上)/u)
    expect(String(parsed.governance.dialogueActKernel.mustSay[0] ?? '')).toMatch(/沿同一条.*(线程|线).*(继续|补上)/u)
