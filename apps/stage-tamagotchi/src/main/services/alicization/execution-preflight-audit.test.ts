...
describe('execution-preflight-audit', () => {
  ...
  it('requires execution-briefing authority files to build canonical project briefing before execution runtime context exists', () => {
    ...
    for (const entry of entries) {
      ...
      expect(source).toContain('function resolveAgentExecutionProjectBriefing(')
      ...
      expect(source).toContain('projectBriefing: identity?.projectBriefing ?? resolveAgentExecutionProjectBriefing(session)')
      expect(source).toContain('const runtimeSurface = session.digitalLifeSpine?.runtimeSurface ?? null')
      expect(source).toContain('sameHerHoldDetail = (() => {')
      expect(source).toContain('const canonicalPreDialogueAwarenessLine = buildAlicizationProjectPreDialogueAwarenessLine({')
      expect(source).toContain(': canonicalPreDialogueAwarenessLine')
      expect(source).toContain('|| surfaceProjectState.preDialogueAwarenessLine')
      expect(source).toContain('|| canonicalProjectBriefing.preDialogueAwarenessLine')
    }
  })
  ...
  it('requires runtime-context authority files to canonicalize project briefing and same-her awareness before dispatch begins', () => {
    ...
    for (const entry of entries) {
      ...
      expect(source).toContain('proactiveSameHerGap: resolvedProactiveSameHerGapInput || null')
  ...
  it('requires runtime-owned dispatch bridge files to rebuild canonical execution runtime context before direct execution redispatch opens outward', () => {
    ...
    for (const entry of entries) {
      ...
      expect(source).toContain('function resolveThreadExperienceProjectBriefing(')
      expect(source).toContain('const normalizedRuntimeContext = normalizeAlicizationExecutionRuntimeContext({')
      expect(source).toContain('projectBriefing: (experience as Record<string, unknown>).projectBriefing ?? null')
      ...
      expect(source).toContain('const experienceProjectBriefing = resolveThreadExperienceProjectBriefing(input.thread)')
      expect(source).toContain('projectBriefing: experienceProjectBriefing ?? undefined')
