...
function analyzeProjectStateAnswerDemand(latestUserText: string) {
  ...
  const asksMergeReadiness
    = /(?:可以|能不能|现在可以|已经可以|can we|is (?:it|this)|ready to|merge-ready).{0,40}(?:合并到\s*main|merge(?:\s+this)?\s+to\s+main|ready to merge)|(?:合并到\s*main|merge(?:\s+this)?\s+to\s+main|ready to merge).{0,24}(?:了吗|吗|now|already|ready|可以|能不能)/iu.test(normalized)
