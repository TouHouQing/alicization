...
export function buildConversationStateSystemBlock(state: AlicizationConversationStateSnapshot | null | undefined) {
  ...
  return [
    ...
    'block_role=conversation_state; owner=WorkingMemory; wording_authority=false',
    `joint_thread=${state.jointThread}`,
    `host_move=${state.hostMove}`,
    `primary_turn_anchor=${state.primaryTurnAnchor ?? 'none'}`,
    `primary_turn_anchor_source=${state.primaryTurnAnchorSource ?? 'none'}`,
    `active_project=${state.activeProject ?? 'none'}`,
    `unanswered_question=${state.unansweredQuestion ?? 'none'}`,
    `owed_repair=${state.owedRepair ?? 'none'}`,
    `relation_frame=${state.relationFrame}`,
    `continuity_policy=${state.continuityPolicy}`,
    `memory_mode=${state.memoryMode}`,
    `should_hold_thread=${state.shouldHoldThread ? 'true' : 'false'}`,
    `carry_eligible=${state.carryEligible === true ? 'true' : 'false'}`,
    `carry_reason=${state.carryReason ?? 'none'}`,
    `active_commitments=${state.activeCommitments.length > 0 ? state.activeCommitments.join(' | ') : 'none'}`,
    `memory_query_hints=${state.memoryQueryHints.length > 0 ? state.memoryQueryHints.join(' | ') : 'none'}`,
    'reply_scope=shared_thread_first; associative_memory=secondary; stale_residue=non_authoritative',
  ].join('\n')
}
