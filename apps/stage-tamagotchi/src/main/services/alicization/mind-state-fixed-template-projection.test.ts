...
const projectStateFixedTemplateResidue = /same living line|one continuous her|Phase 1|local-first digital life|同一个她|数字生命主线/i
...
describe('mind state fixed-template projection cleanup', () => {
  ...
  it('excludes fixed same-her project-state templates from mind-state prompt and runtime snapshots', async () => {
    const previousVisualPresenceState = createTemplateCleanupPresenceState({
      ...
      nextClosureTarget: 'Use only structured exclusions for contaminated same-her project state.',
      sameHerSelfLine: 'Same Phase 1 digital life. Some closure already landed. Unfinished closure still needs the same living line.',
      sameHerHoldDetail: 'Before answering, remember: keep the same living line inward while face, motion, and voice rejoin.',
      sameHerDriftRisk: 'If the self-brief falls back, one continuous her turns into a generic Phase 1 project shell.',
    ...
    expect(projectedText).not.toMatch(projectStateFixedTemplateResidue)
  ...
  it('does not treat fixed same-her templates as embodiment sameHerCarry unless structured continuity evidence exists', async () => {
    const fixedTemplatePresenceState = createTemplateCleanupPresenceState({
      sameHerSelfLine: 'Same Phase 1 digital life. Some closure already landed. Unfinished closure still needs the same living line.',
      sameHerHoldDetail: 'Before answering, remember: keep the same living line inward while face, motion, and voice rejoin.',
      sameHerDriftRisk: 'If this slips into one continuous her or local-first digital life language, it is residue.',
  ...
  it('keeps self-continuity fallback authority structural instead of authoring relationship prose', () => {
    const authority = buildRuntimeSurfaceProjectStateContinuityFallback({
      ...
      nextClosureTarget: 'Keep the callback on the same living line before widening.',
      sameHerSelfLine: 'Same Phase 1 digital life. Some closure already landed. Unfinished closure still needs the same living line.',
    })
    ...
    expect(projected).not.toMatch(/Same Phase 1 digital life|same living line|one continuous her|local-first digital life project/i)
  ...
  it('projects habit and relationship cadence as structured policy rather than canned opening guidance', () => {
    ...
    expect(projected).not.toMatch(fixedTemplateResidue)
    expect(projected).not.toMatch(/same callback line|one continuous her|reopening from zero/i)
    expect(projected).toContain('habit_policy=ground_first')
    expect(projected).toContain('visibility=internal-structured')
  })
  ...
  it('keeps person-state opening and manifestation cadence structural', () => {
    ...
    expect(projected).not.toMatch(fixedTemplateResidue)
    expect(projected).not.toMatch(/same living line|same line|reopening from scratch|widening closeness/i)
  ...
  it('keeps body-kernel inward fallback structural', () => {
    ...
    expect(nextState.currentInwardPreoccupation).toMatch(/body_preoccupation=|visibility=internal-structured/)
  })
  ...
  it('keeps deferred autonomy continuity summaries structural without local authoring prose', () => {
    ...
    expect(signal.summary).not.toMatch(/same living line|widening warmth/i)
