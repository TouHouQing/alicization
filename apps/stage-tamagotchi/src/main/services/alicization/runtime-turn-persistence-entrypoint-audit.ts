...
const appendConversationTurnWithGuardsNeedle = 'appendConversationTurnWithGuards('

export function collectAlicizationRuntimeTurnPersistenceFiles(rootDir: string) {
  ...
  while (queued.length > 0) {
    ...
    for (const entry of readdirSync(currentDir, { withFileTypes: true })) {
      ...
      if (source.includes(appendConversationTurnWithGuardsNeedle))
