...
describe('humanlike memory recall priority regression', () => {
  ...
  it('carries remembered initiative rhythm cues into recollection intent so later initiative can inherit window, anti-spam, and visible reopening cadence', () => {
    const recallSeed = [
      'humanlike_memory_recall: line=initiative_recall=low_pressure_follow_up; status=unresolved; opening=wait_for_relevant_window; visibility=memory_structured | relationship=The same-person continuity line is still open, but it should remember the gentler reopening cadence that was earned. | emotion=protective-continuity,unfinishedness | initiative=low-pressure-follow-up | initiative_window=next corrected continuity reopening when the host is already re-entering the same line | initiative_pressure=low | initiative_anti_spam=Do not turn same-person continuity into timer spam; wait until the line is visibly reopening on its own. | initiative_visible_policy=opening=host_reopen_required; pressure=low; visibility=memory_structured | embodiment=Reply should stay quieter while remembering the earned reopening cadence. | self=I learned to remember not just whether to return, but the gentler rhythm that lets the same line reopen without crowding. | why=the reopening cadence itself changed after repeated same-person continuity work | created=86000',
    ].join('\n')
    ...
    expect(recollectionIntent?.queryHints).toEqual(expect.arrayContaining([
      ...
      'initiative_visible_policy=opening=host_reopen_required; pressure=low; visibility=memory_structured',
    ]))
    expect(recollectionIntent?.recollectionAgenda?.candidateProcedureLines).toEqual(expect.arrayContaining([
      'initiative_window=next corrected continuity reopening when the host is already re-entering the same line',
      'initiative_anti_spam=Do not turn same-person continuity into timer spam; wait until the line is visibly reopening on its own.',
  ...
  it('keeps afterglow-triggered recall intent structured so provider-facing rationale cannot become fixed same-line wording', () => {
    const recallSeed = [
      'continuity_afterglow: label=execution-callback-afterglow | summary=thread=runtime | continuity=execution-callback | carry-mode=lower-pressure | carry=provider callback should stay procedural before generic background history | source=runtime | provenance=runtime',
    ].join('\n')
    ...
    const providerFacingText = [
      recollectionIntent?.rationale,
      recollectionIntent?.recollectionAgenda?.whyRecallNow,
      ...(recollectionIntent?.recollectionAgenda?.candidateTimeScopes ?? []).map(item => item.rationale),
      ...(recollectionIntent?.recollectionAgenda?.candidateEraFacets ?? []).map(item => item.rationale),
    ].filter(Boolean).join('\n')

    expect(recollectionIntent?.mode).toBe('execution-procedure')
    expect(providerFacingText).toContain('callback_afterglow')
    expect(providerFacingText).not.toMatch(/same carried line|same carried seam|same line restarts|same line reopened|unfinished working seam/iu)
  })
})
